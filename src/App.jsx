...
function status(v) { return v < 70 ? 'Low' : v > 140 ? 'High' : 'In range'; }
...
function D3Chart({ data }) {
  ...
  const maxY = Math.max(190, (d3.max(values, d => Number(d.value)) || 140) + 10);
...
export default function App() {
  ...
  const stats = useMemo(() => {
    ...
    const inRange = rows.filter(d => d.value >= 70 && d.value <= 140).length;
    const highs = rows.filter(d => d.value > 140).length;
